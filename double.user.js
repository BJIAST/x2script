...
function steamlink(){
    console.log($("#inlineAlert").html());
    if ($("#inlineAlert").html() == '<i class="fa fa-exclamation-triangle"></i> Attempting mobile confirmation: success'){
        $('#offerContent b a')[0].click();
    };
    if ($("#inlineAlert").html() == '<i class="fa fa-exclamation-triangle"></i> Attempting mobile confirmation: failed'){
        $this.click();
        steamlink();
    }
if ( $("#inlineAlert").html() ==  'Offer not found in pending offer pool for "'+ name +'" - if your Steam name has changed please re-login and try again.'){
    setTimeout(function () {
        $this.click();
        steamlink();
    },5000);
                        ...
                        if (data.success) {
                            ...
                            switch(data.error){
                                ...
                                case 'Offer not found in pending offer pool for "'+ name +'" - if your Steam name has changed please re-login and try again.' :
                                    setTimeout(function(){ $this.click(); }, 5000);
                                    break;
                                case "<i class='fa fa-exclamation-triangle'></i> Attempting mobile confirmation: success":
                                case "This offer is still pending. Please accept the trade offer and try again.":
                                    document.title = "GO TO STEAM!";
                                    console.log(data.error);
                                    showlogs("Можно принять офер в стиме!");
