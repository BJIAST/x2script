...
$(document).ready(function() {
    ...
    if(confirm("Стандартная загрузка (ок) или забераем кейсы (отмена)")) {
        (function () {
    'use strict';
    ...
    $(document).ajaxSend(
            function(){
                alert("На сервер только что был отправлен ajax-запрос." + settings.url);
            }
        );
...
function DrawSettingsPanel(){
    $('body').append("<div id='SettingsPanel' style='z-index:2000;position:fixed;width:100px;;top:0;color:black; border:2px solid; background:orange;'>" +
                     "<span onClick='App.DisplayAllItems()'><a href=''>Показать все шмотки</a></span><br>"+
